  ...
  function renderLearnMovements(){
    state.screen = 'learn-movements';
    updateChrome();
    screenEl.innerHTML = `
      <div class="stage-wrap">
        <h2 class="stage-title">movements</h2>
        <div class="mode-list" id="movementList"></div>
        <div class="stage-actions">
          <button class="btn btn-ghost" id="backBtn">back</button>
        </div>
      </div>`;
    const list = document.getElementById('movementList');
    LEARN_MOVEMENTS.forEach(name => {
      const b = document.createElement('button');
      b.className = 'mode-item';
      b.innerHTML = `<span class="mode-main"><span class="mode-name">${name}</span></span>`;
      b.addEventListener('click', () => renderLearnMovementDetail(name));
      list.appendChild(b);
    });
    document.getElementById('backBtn').addEventListener('click', renderLearn);
  }

  function renderLearnMovementDetail(name){
    state.screen = 'learn-movement-detail';
    updateChrome();
    screenEl.innerHTML = `
      <div class="stage-wrap">
        <h2 class="stage-title">${name.toLowerCase()}</h2>
        <p class="stage-body">Content coming soon.</p>
        <div class="stage-actions">
          <button class="btn btn-ghost" id="backBtn">back</button>
        </div>
      </div>`;
    document.getElementById('backBtn').addEventListener('click', renderLearnMovements);
  }

  function renderLearnArtists(){
    state.screen = 'learn-artists';
    updateChrome();
    screenEl.innerHTML = `
      <div class="stage-wrap">
        <h2 class="stage-title">artists</h2>
        <p class="stage-body">Content coming soon.</p>
        <div class="stage-actions">
          <button class="btn btn-ghost" id="backBtn">back</button>
        </div>
